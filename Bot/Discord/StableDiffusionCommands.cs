...
namespace IntelChan.Bot.Discord
{
    ...
    {
        [SlashCommand("draw", "Draw an image using Stable Diffusion")]
        public async Task Draw(string positivePrompt, string negativePrompt = "nsfw, nudity", int width = 512, int height = 512, string sampler = "Euler a", IAttachment? image = null)
        {
            ...
            if (resp.StatusCode == System.Net.HttpStatusCode.OK)
            {
                ...
                if (imgResp != null)
                {
                    ...
                    await RespondWithFileAsync("sd.png");
                }
            }
            await RespondAsync("Did not receive a valid response from Stable Diffusion. Is the service down?");
        }
    }
}